...
describe("🚀 Endpoint Resilience Testing", () => {
  ...
  it("Screener harus auto-batch kalau >15 symbols", async () => {
    ...
    console.log(`✅ Screener response status: ${res.status}, data length: ${res.body?.data?.length || 'N/A'}`);
    ...
    if (res.body.metadata?.batch_info) {
      console.log(`📦 Batching used: ${res.body.metadata.batch_info.batching_used}`);
      expect(res.body.metadata.batch_info.batching_used).toBe(true);
    }
  });
  ...
  it("CoinAPI History harus retry kalau response incomplete", async () => {
    ...
    console.log(`✅ History response status: ${res.status}, data length: ${res.body?.length || 'N/A'}`);
  ...
  it("Health check endpoint should be responsive", async () => {
