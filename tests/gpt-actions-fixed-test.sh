...
# GPT Actions Domain Test Script (FIXED)
# Tests all GPT Actions endpoints with correct parameters

DOMAIN="https://guardiansofthetoken.com"
TIMESTAMP=$(date +%Y%m%d-%H%M%S)
LOG_FILE="/var/log/crypto-api/gpt-actions-fixed-${TIMESTAMP}.log"
...
echo "========================================" | tee -a "$LOG_FILE"
echo "GPT ACTIONS DOMAIN TEST (FIXED)" | tee -a "$LOG_FILE"
...
test_endpoint "Unified Advanced (whale_alerts)" "POST" "/gpts/unified/advanced" '{"op":"whale_alerts","symbol":"BTC","exchange":"hyperliquid"}' ".ok"
test_endpoint "Unified Advanced (market_sentiment)" "POST" "/gpts/unified/advanced" '{"op":"market_sentiment","symbol":"BTC"}' ".ok"
test_endpoint "Unified Advanced (volume_spikes)" "POST" "/gpts/unified/advanced" '{"op":"volume_spikes","symbol":"BTC"}' ".ok"
echo "" | tee -a "$LOG_FILE"
...
test_endpoint "Institutional Bias BTC" "GET" "/gpts/institutional/bias?symbol=BTC" "" ".data.bias"
test_endpoint "Institutional Bias ETH" "GET" "/gpts/institutional/bias?symbol=ETH" "" ".data.bias"
echo "" | tee -a "$LOG_FILE"

echo "=== GPT HEALTH ENDPOINTS ===" | tee -a "$LOG_FILE"
test_endpoint "GPT Health" "GET" "/gpts/health" "" ".endpoints"
test_endpoint "CoinAPI Health" "GET" "/gpts/health/coinapi" "" ".services"
echo "" | tee -a "$LOG_FILE"
