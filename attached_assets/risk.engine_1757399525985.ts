...
// Drop-in for screening-module v2.x
// ======================================================================
...
const ceilStep = (v:number, step:number) => Math.ceil(v/step)*step;
const floorStep = (v:number, step:number) => Math.floor(v/step)*step;
...
export function riskEngine(params:{
  ...
  if (!last) throw new Error("No last close");
  const entry = params.entryPrice ?? last;

  const atr = computeATR(candles, 14);
  if (!atr) throw new Error("ATR not available");

  // raw SL/TP using ATR multiples
  const slRaw = side==='long' ? entry - risk.atrSLMult*atr : entry + risk.atrSLMult*atr;
  const rrUnit = Math.abs(entry - slRaw);

  const tp1Raw = side==='long' ? entry + risk.tp1RR*rrUnit : entry - risk.tp1RR*rrUnit;
  const tp2Raw = side==='long' ? entry + risk.tp2RR*rrUnit : entry - risk.tp2RR*rrUnit;

  // Adjust for price tick size
  ...
  const capNotional = risk.capPositionPct ? risk.accountEquity * (risk.capPositionPct/100) : Infinity;
  const maxQtyByCap = capNotional / entry;
  let qty = Math.min(qtyRaw, maxQtyByCap);

  // exchange constraints: minQty, step, minNotional
  ...
  // R/R (gross) based on distances (ignoring costs in the RR ratio but costs are reported separately)
  const rr1 = rrUnit>EPS ? (Math.abs(tp1 - entry) / rrUnit) : 0;
  const rr2 = rrUnit>EPS ? (Math.abs(tp2 - entry) / rrUnit) : 0;

  // validations
  const violations:string[] = [];
  if (notional < exch.minNotional) violations.push(`Notional ${notional.toFixed(2)} < minNotional ${exch.minNotional}`);
  if (qty < exch.minQty) violations.push(`Qty ${qty} < minQty ${exch.minQty}`);
  if (rr1 < 1.0) violations.push(`RR1 ${rr1.toFixed(2)} < 1.0`);
  if (sl >= entry && side==='long') violations.push(`SL (${sl}) invalid for long`);
  if (sl <= entry && side==='short') violations.push(`SL (${sl}) invalid for short`);
